...
int sock_send(sock_t *sock, void *buffer, size_t size) {
  ...
  worker->type.sender.buffer = buffer; worker->type.sender.size = size;
...
static void *sock_thread_messenger(void *arg) {
  ...
  if (NULL != sock->cb.message.fct) {
    ...
    sock->cb.message.fct(sock, worker->type.messenger.ip, worker->type.messenger.port, worker->type.messenger.buffer, worker->type.messenger.size, sock->cb.message.user);
  }
      ...
      while (NULL != pch) {
        ...
        for (int index = 0; index < FD_SETSIZE; index++) {
          if (FD_ISSET(index, &sock->clients.fds)) {
            if (worker->type.sender.size != sendto(index, worker->type.sender.buffer, worker->type.sender.size, 0, (struct sockaddr *)&addr, sizeof(addr))) {
              /* Unable to send data */
            }
          }
        }
